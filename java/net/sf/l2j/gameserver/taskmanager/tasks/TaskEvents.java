...
{
	if (TvT._joining || TvT._started || TvT._teleport || CTF._joining || CTF._started || CTF._teleport || FOS._joining || FOS._started || FOS._teleport || VIP._joining || VIP._started || VIP._sitForced || DM._joining || DM._started || DM._sitForced)
		return;
	...
	else  //30% for FOS
	{
		...
		{
			if (Rnd.get(2) == 0)
				FOS.loadData("Aden Castle Event");
			else
				FOS.loadData("Rune Castle Event");
			FOS.autoEvent();
		}
	}
}
