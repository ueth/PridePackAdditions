...
private static final double[] STRCompute = new double[] { 1.036, 34.845 }; // {1.016, 28.515};
// for C1
private static final double[] INTCompute = new double[] { 1.020, 31.375 }; // {1.020, 31.375};
// for C1
private static final double[] DEXCompute = new double[] { 1.009, 19.360 }; // {1.009, 19.360};
// for C1
private static final double[] WITCompute = new double[] { 1.030, 19.800 }; // {1.040, 19.000};
// for C1
private static final double[] CONCompute = new double[] { 1.030, 27.552 }; // {1.015, 12.488};
// for C1
private static final double[] MENCompute = new double[] { 1.010, -0.060 }; // {1.010, -0.060};
// for C1
...
public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean dual, boolean ss)
{
	...
	if (crit)
	{
		...
		damage = attacker.getCriticalDmg(target, 1.66, skill)
	...
	else if (target instanceof L2PcInstance)
	{
		/*if (attacker instanceof L2RaidBossInstance)
			{
				final L2Clan clan = target.getActingPlayer().getClan();
				if (clan != null && clan.getRBkills() >= 8)
				{
					if (!target.getActingPlayer().isGM())
					{
...
public static final boolean calcAtkBreak(L2Character attacker, L2Character target, int damage)
{
	...
	if (attacker instanceof L2Attackable || damage >= dmgThreashold)
	{
		...
		else if (Config.ALT_GAME_CANCEL_BOW && damage >= dmgThreashold && target.isAttackingNow() &&
				target.getActiveWeaponItem() != null &&
				(target.getActiveWeaponItem().getItemType() == L2WeaponType.BOW || target.getActiveWeaponItem().getItemType() == L2WeaponType.CROSSBOW))
		{
	...
	if (attacker instanceof L2Playable && !attacker.getActingPlayer().isInOlympiadMode())
	{
		...
		if (overpower > 0)
		{
			...
			if (overpower > 0)
			{
				try
				{
					if ((target.isAttackingNowOverpower() && target.getAI().getAttackTarget() == attacker) ||
							(target.isCastingNow() && target.getAI().getSkill().isDamaging() && target.getAI().getCastTarget() == attacker))
					{
...
public static double calcSkillProficiency(L2Skill skill, L2Character attacker, L2Character target)
{
	...
	if (skill != null)
	{
		...
		// For additional effects on PDAM and MDAM skills (like STUN, SHOCK, PARALYZE...)
		if (type != null && (type == L2SkillType.PDAM || type == L2SkillType.PDAMPERC ||type == L2SkillType.MDAM || type == L2SkillType.DRAIN || type == L2SkillType.BLOW
				|| type == L2SkillType.DEATHLINK || type == L2SkillType.CHARGEDAM || type == L2SkillType.CPDAM  || type == L2SkillType.FATAL))
			type = skill.getEffectType();
...
public static int calcEffectSuccessChance(L2Character attacker, L2Character target, EffectTemplate effect, L2Skill skill, byte shld)
{
	...
	if (skill.isMagic())
	{
		if (attacker instanceof L2PcInstance && !attacker.getActingPlayer().isMageClass())
			rate = (int) (rate * Math.pow((double) attacker.getMAtk(target, skill) + 9000
					/ (target.getMDef(attacker, skill) + (shld == 1 ? target.getShldDef() : 0)), 0.2));
		else
			rate = (int) (rate * Math.pow((double) attacker.getMAtk(target, skill) + 4000
					/ (target.getMDef(attacker, skill) + (shld == 1 ? target.getShldDef() : 0)), 0.2));
	}
	...
	if (target != null && (target.getFirstEffectById(287) != null || target.getFirstEffectById(499) != null ||
			(target instanceof L2PcInstance && target.getActingPlayer().getCurrentSkill() != null && target.getActingPlayer().getCurrentSkill().getSkillId() == 246)))
		lionheart = true;
...
public static int calcSkillSuccessChance(L2Character attacker, L2Character target, L2Skill skill, byte shld)
{
	...
	if (skill.isMagic())
	{
		if (attacker instanceof L2PcInstance && !attacker.getActingPlayer().isMageClass())
			...
			rate = (int) (rate * Math.pow((double) attacker.getMAtk(target, skill) + 5000
					/ (target.getMDef(attacker, skill) + (shld == 1 ? target.getShldDef() : 0)), 0.2));
	}
	...
	if (target.getFirstEffectById(287) != null || target.getFirstEffectById(499) != null ||
			(target instanceof L2PcInstance && target.getActingPlayer().getCurrentSkill() != null && target.getActingPlayer().getCurrentSkill().getSkillId() == 246))
		lionheart = true;
...
public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, L2Character target, L2Skill skill, byte shld)
{
	...
	if (skill.isMagic())
		rate += (int) (Math.pow((double) attacker.getMAtk()
				/ (target.getMDef(attacker.getOwner(), skill) + (shld == 1 ? target.getShldDef() : 0)), 0.2) * 100) - 100;
...
public static double calcElemental(L2Character attacker, L2Character target, L2Skill skill)
{
	...
	{
		if (attacker.getActingPlayer().isInOrcVillage() || (DM._started && attacker.getActingPlayer()._inEventDM))
			return 1;
...
/**
 * Calculate skill reflection according these three possibilities: <li>Reflect failed</li> <li>
 * Mormal reflect (just effects). <U>Only possible for skilltypes: BUFF, REFLECT, HEAL_PERCENT,
 * MANAHEAL_PERCENT, HOT, CPHOT, MPHOT</U></li> <li>vengEance reflect (100% damage reflected but
