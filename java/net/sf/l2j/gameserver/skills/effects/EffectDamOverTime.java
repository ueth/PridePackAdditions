...
public boolean onActionTime()
{
	...
	if (damage >= getEffected().getCurrentHp() - 1)
	{
		...
		if (!getSkill().killByDOT())
		{
			...
			if (getEffector() instanceof L2PcInstance && getEffected() instanceof L2RaidBossInstance)
			{
				getEffector().incDamageCount((int)damage); 
				getEffected().addRaidAttackerToList(getEffector());
			}
		}
	}
