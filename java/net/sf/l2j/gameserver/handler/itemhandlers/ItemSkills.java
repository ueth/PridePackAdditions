...
public void useItem(L2Playable playable, L2ItemInstance item)
{
	...
	if(Manager.getInstance().isPlayerRegistered(activeChar))
	{
		activeChar.sendMessage("You are already registered for Fair Games.");
		return;
	}
			...
			if (skill != null && skill.length == 2)
			{
				...
				if (skillId > 0 && skillLvl > 0)
				{
					...
					if (itemSkill != null)
					{
						...
						if (playable.getActingPlayer().isInOrcVillage())
						{
							if (itemSkill.getTransformId() > 0)
							{
								playable.sendMessage("You can't transform in Orc Village");
								return;
							}
						}
