...
public boolean useAdminCommand(String command, L2PcInstance activeChar)
{
	...
	else if (command.startsWith("admin_fence"))
	{
		int length = 90;
		int width = 90;
		int type = 2;
		...
		{
			activeChar.sendMessage("Usage: //fence length width (length and width are optional");
			return false;
		}
		
		spawnFence(activeChar, type, length, width);
	}
	return true;
}

private void spawnFence(L2PcInstance activeChar, int type, int length, int width)
{
	L2FenceInstance fence = new L2FenceInstance(IdFactory.getInstance().getNextId(), type, length, width, activeChar.getX(), activeChar.getY(), activeChar.getZ(), 0);
	fence.setInstanceId(activeChar.getInstanceId());
...
private void spawnMonster(L2PcInstance activeChar, String monsterId, int respawnTime, int mobCount, boolean permanent, int dayNight, int location)
{
	...
	try
	{
		...
		if (RaidBossSpawnManager.getInstance().isDefined(spawn.getNpcid()) || isApcNotBoss(spawn))
		{
			if(isApcNotBoss(spawn))
				activeChar.sendMessage("APC: "+ template1.name + " will be spawned temporarily.");
			else			
...
private boolean isApcNotBoss(L2Spawn spawn)
{
