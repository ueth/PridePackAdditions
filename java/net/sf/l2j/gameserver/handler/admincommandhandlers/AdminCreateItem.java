...
public boolean useAdminCommand(String command, L2PcInstance activeChar)
{
	...
	else if (command.startsWith("admin_rewardal"))	//Meydex
	{		
		try
		{
			String val = command.substring(15);
			...
			if (st.countTokens() == 2)
			{
				...
				String num = st.nextToken();
				long numval = Long.parseLong(num);
				rewardAll(activeChar, idval, numval);
			}
			else
			{
				activeChar.sendMessage("Usage: //rewardal <itemId> <amount>");
			}
		}
		catch (StringIndexOutOfBoundsException e)
		{
...
private void rewardAll(L2PcInstance activeChar, int id, long num)
{
    final L2Item template = ItemTable.getInstance().getTemplate(id);
    ...
    for (L2PcInstance p : ppl)
    {
        ...
        if(!p.isInsideRadius(activeChar, 5000, false, false))
        	continue;
        ...
        {               	       	        	
        	if (activeChar != p)
         		p.sendMessage("GM has given you " + num + " " + itemName + " in your inventory.");
        	p.addItem("giveItem.", id, num, p, true);                                               	                       	                        	
        }
        else
            p.sendMessage("Only the character from the first opened tab gets rewards, so this character won't be rewarded.");
        
        ItemList il = new ItemList(p, true);
     	p.sendPacket(il);
    }
	announce("Everyone near the GM got automatically rewarded!");
	activeChar.sendMessage("You have spawned " + num + " item(s) number " + id + " ("+itemName+") in everyone's inventory in 5000 radius!");
	playerIps.clear();      		 	
}
