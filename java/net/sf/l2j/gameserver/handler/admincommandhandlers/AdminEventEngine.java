...
public class AdminEventEngine implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_pvp_event"))
		{
		    ...
		    for (L2PcInstance p : ppl)
		    {
		    	...
		    	p.setInGMPvPEvent(true);
		    }
		    Broadcast.announceToOnlinePlayers("x10 PvP reward event has started! During the event every time you get 1 pvp point it counts as 10!");
		}
		else if (command.startsWith("admin_stop_pvp_event"))
		{
			Collection<L2PcInstance> ppl = L2World.getInstance().getAllPlayers().values();
		    for (L2PcInstance p : ppl)
		    {
		    	if (p.getClient() == null || p.getClient().isDetached()) // offline shops protection
		            continue;		    	
		    	p.setInGMPvPEvent(false);
		    }
		    Broadcast.announceToOnlinePlayers("x10 PvP reward event has ended."); //TODO: announce top killer during the event
		}
		return true;
	}
	...
	public void showNewEventPage(L2PcInstance activeChar)
	{
		...
		if (tempBuffer.isEmpty())
			replyMSG.append("Use //add text or //delete_buffer to modify this text field");
		else
			replyMSG.append(tempBuffer);
		
		if (!(tempName.isEmpty() && tempBuffer.isEmpty()))
			replyMSG.append("<br><button value=\"Clear\" action=\"bypass -h admin_event_store\" width=90 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
