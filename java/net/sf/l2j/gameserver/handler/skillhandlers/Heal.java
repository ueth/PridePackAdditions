...
public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
{
	...
	for (L2Character target: (L2Character[]) targets)
	{
		...
		if (target != activeChar)
		{
			if (target instanceof L2PcInstance && ((L2PcInstance) target).isCursedWeaponEquipped())
				...
				continue;
		                                         ...
		                                         //**Custom Edit** Heal is influenced by M.Atk
		double hp = skill.getPower(activeChar) + ((activeChar.getMAtk(target, skill))/100);
		...
		//IP CUSTOM NERF
		/*if (target instanceof L2PcInstance && ((L2PcInstance) target).getClient().getConnection().getInetAddress().getHostAddress() == player.getClient().getConnection().getInetAddress().getHostAddress() && ((L2PcInstance) target) != player)
		{
			hp /= 2;
		}*/
