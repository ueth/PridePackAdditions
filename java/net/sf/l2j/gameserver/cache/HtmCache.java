...
public class HtmCache {
	...
	public String getHtm(final String prefix, final String path) {
		String newPath = null;
		if (prefix != null && !prefix.isEmpty()) {
			newPath = prefix + path;
			final String content = this.getHtm(newPath);
			if (content != null) {
				return content;
			}
		}
		String content = this.getHtm(path);
		if (content != null && newPath != null) {
			content = "<html><body>My text is missing:<br>" + path + "</body></html>";
			HtmCache._log.warning("Cache[HTML]: Missing HTML page: " + path);
		}
		return content;
	}
