...
public class FenceTable 
{
	...
	public List<L2FenceInstance> getFences(int mapId)
	{
		...
		{
			fences.add(new L2FenceInstance(IdFactory.getInstance().getNextId(), f.type, f.width, f.length, f.x, f.y, f.z, mapId));
		}
		return fences;
	}
	...
	public void spawnFences(List<L2FenceInstance> list, int instance)
	{
		for(L2FenceInstance fence : list)
		{
			if(fence != null)
			{
				...
				fence.spawnMe(fence.getXLoc(), fence.getYLoc(), fence.getZLoc());
			}
		}
	}
