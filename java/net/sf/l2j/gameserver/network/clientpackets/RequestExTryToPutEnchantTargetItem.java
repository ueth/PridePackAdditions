...
protected void runImpl()
{
	...
	if (activeChar != null)
	{
		...
		if (chance >= 100)
		{
			ConfirmDlg dlg = new ConfirmDlg(SystemMessageId.S2.getId()).addString("Success chance: "+chance+"% - Max enchant: +"+scrollTemplate.getMaxEnchantLevel(item));
			activeChar.sendPacket(dlg);
		}
		else
		{
			ConfirmDlg dlg = new ConfirmDlg(SystemMessageId.S2.getId()).addString("Success chance: "+chance+"% - Failure penalty: "+penalty+" - Max enchant: +"+scrollTemplate.getMaxEnchantLevel(item));
			activeChar.sendPacket(dlg);
		}
