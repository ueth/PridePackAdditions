...
protected void runImpl()
{
	...
	case 22: // pet attack
		if (target != null && pet != null && pet != target && !pet.isAttackingDisabled() && !pet.isBetrayed())
		{
			...
			{
				if (!activeChar.isInFunEvent() || !((L2Playable)target).isInFunEvent())
				{
	...
	case 65:
		...
		if (getClient().reportTickTimer >= ticks)
		{
			sendPacket(ActionFailed.STATIC_PACKET);
		...
		getClient().reportTickTimer = ticks + 100;
		/*				String ip1 = activeChar.getIP();
				String ip2 = ((L2PcInstance)target).getIP();*/
		
		String msgContent = activeChar.getName() + " has submitted a bot report on player " + activeChar.getTarget().getName() + ". Location: "+ activeChar.getTarget().getX() + ", " + activeChar.getTarget().getY()
				+ ", " + activeChar.getTarget().getZ() + ".";
		GmListTable.broadcastToGMs(new CreatureSay(activeChar.getObjectId(), 17, "Bot System", msgContent));
