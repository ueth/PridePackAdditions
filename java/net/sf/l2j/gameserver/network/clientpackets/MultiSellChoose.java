...
public void runImpl()
{
	...
	{				
		...
		{
			doExchange(player, entry, list.getApplyTaxes(), list.getMaintainEnchantment(), _enchantment, allowSell, allowSellMulti);
			return;
		}
	}
}

private void doExchange(L2PcInstance player, MultiSellEntry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantment, boolean allowSell, double allowSellMulti)
{
	...
	if (!player.isGM())
	{
		...
		for (MultiSellIngredient e : entry.getProducts())
		{
			...
			if (template == null)				
				continue;
			/*if (!template.isStackable() && (_amount > 1))
				continue;*/
	...
	for (MultiSellIngredient e : _ingredientsList)
	{
		...
		default:
		{
			...
			if (inventoryCount < ((Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMantainIngredient()) ? (e.getItemCount() * _amount) : e.getItemCount()))
			{
			...
			if (inv.getAllItemsByItemIdMultisell(e.getItemId(), e.getEnchantmentLevel())[0].isEnchantable())
			{
	...
	boolean usedMedalOfVictory = false;
	for (MultiSellIngredient e : entry.getIngredients())
	{
		switch (e.getItemId())
		{
		...
		default:
		{
			...
			if (e.getItemId() == TvT.MEDAL_OF_VICTORY)
				usedMedalOfVictory = true;
					...
					else
					{
						if (e.getEnchantmentLevel() >= 1 && product.isEquipable())
						{
							if (!swapped && !(merchant instanceof L2DonationMerchantInstance) && merchant.getNpcId() != 90004)
							{
								...
								{
									if (chance < 1 && product.getUniqueness() < 3)
										motherluck = 5;
									else if (chance < 5 && product.getUniqueness() < 4)
										motherluck = 4;
									else if (chance < 15)
										motherluck = 3;
									else if (chance < 30)
