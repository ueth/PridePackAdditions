...
{
	_objectId = 0;
	try { _objectId = readD(); } catch (Exception e) { }
}
...
protected void runImpl()
{
	...
	if (item.getItem().getItemType() == L2WeaponType.ROD || item.isShadowItem()
			/*|| item.isHeroItem()*/ /*|| item.isTimeLimitedItem()*/ || item.isEtcItem() || item.isWear() || item.getItem().getBodyPart() == L2Item.SLOT_L_BRACELET
			...
			|| (item.getItem().getItemGradeSPlus() < L2Item.CRYSTAL_D && item.getItemId() != 20325)
	...
	if (!stone.isTradeable() && stone.getUntradeableTime() > System.currentTimeMillis())
	{
		if (item.getUntradeableTime() < stone.getUntradeableTime())
		{
			...
			player.sendMessage("Your "+item.getName()+" is now untradeable for "+stone.getUntradeableTime()/3600000+"" +
					" hours due to your element stone being temporarily untradeable");
		}
	}
