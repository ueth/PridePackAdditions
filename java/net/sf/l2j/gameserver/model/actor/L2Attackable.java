...
public class L2Attackable extends L2Npc {
            ...
            if (this instanceof L2MinionInstance) {
                ...
                if (master != null && !master.isDead() && (!master.isInCombat() || isAPC())) {
                    ...
                    if (player != null && attackTarget != null && attackTarget instanceof L2Playable) {
                        if (!(player.getInSameClanAllyAs(attackTarget.getActingPlayer()) == 2 || ((player.getParty() != null && player.getParty().getPartyMembers()
                                .contains(attackTarget.getActingPlayer())))))
                            damage /= 2;
                    }
    ...
    private void handleBattlePassPoints(List<L2PcInstance> players){
        /**
        ...
        for(L2PcInstance player : players){
            for (BattlePass battlePass : player.getBattlePass().getBattlePasses())
                battlePass.increasePoints(false, hp, pdef, mdef, patk, matk);
        }
    }
    ...
    protected void calculateRewards(L2Character lastAttacker) {
        ...
        FastList<L2PcInstance> peopleToRewardExpSp = new FastList<>();
        try {
    ...
    private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep) {
        ...
        if (itemCount > 0)
            return new RewardItem(itemId, itemCount, drop.getEnchantLevel(), drop.getPartyDropCount());
        ...
        if (npcTemplate.getDropData() != null) {
            for (L2DropCategory cat : npcTemplate.getDropData()) {
                ...
                if (cat.isSweep()) {
                    ...
                    if (isSeeded()) {
                    ...
                    } else { //DropAllFromCategory

                        FastList<RewardItem> dropList = new FastList<RewardItem>();
                        ...
                        for (L2DropData drop : cat.getAllDrops()) {
                            ...
                            if (item == null) {
                                ...
                                if (isFlying() || (!isRaid() && Config.AUTO_LOOT) || (isRaid() && Config.AUTO_LOOT_RAIDS) || item.getPartyDropCount() >= 1 || item.getItemId() == 98020
                                        || item.getItemId() == 98021 || item.getItemId() == 98022)
                    ...
                    if (item != null && isSeeded()) //DropAllFromCategory
                    {
                        final int id = item.getItemId();
     ...
     *
     * @param attacker The L2Character who hit on the L2Attackable using the over-hit
     *                 enabled skill
    ...
    public void onSpawn() {
        ...
        if (Config.L2JMOD_CHAMPION_ENABLE) {
            ...
            if (this instanceof L2MonsterInstance && !getTemplate().isQuestMonster && !isRaid() && !isRaidMinion() && Config.L2JMOD_CHAMPION_FREQUENCY > 0 && getLevel() >= Config.L2JMOD_CHAMP_MIN_LVL
                    && getLevel() <= Config.L2JMOD_CHAMP_MAX_LVL && getTemplate().baseHpMax < 1000000 && !isAPC()) {
    ...
    public boolean isLevelOneRaidboss() {
        return (this instanceof L2RaidBossInstance && getLevel() == 85 && getNpcId() != 25325);
    }

    public boolean isLevelTwoRaidboss() {
        return (this instanceof L2RaidBossInstance && getLevel() == 91 && getNpcId() != 25325);
    }
