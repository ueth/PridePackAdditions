...
public boolean doDie(L2Character killer)
{
	...
	if (player != null/* && this instanceof L2PcInstance*/)
	{
		...
		if (awardPvp)
		{
			...
			{
				if ((player.isInOrcVillage() || actingPlayer.isInOrcVillage()) && (player.getPvpKills() >= 3000 && ((float)actingPlayer.getPvpKills()/(float)player.getPvpKills() < 0.4 ||
...
public boolean checkIfPvP(L2Character target)
{
	...
	if (targetPlayer.getPvpFlagLasts() < System.currentTimeMillis() - 2000) return false;
...
public final boolean isInFunEvent()
{
	...
	if (_owner != null)
	{
		...
		return (_owner.atEvent || (TvT._started && _owner._inEventTvT) || (FOS._started && _owner._inEventFOS) || (DM._started && _owner._inEventDM) || (CTF._started && _owner._inEventCTF)
				|| (VIP._started && _owner._inEventVIP) || _owner._inEventTvTi);
	}
