...
public int getAccuracy(L2Character target)
{
	...
	int finalAccuracy = (int)(calcStat(Stats.ACCURACY_COMBAT, 0, target, null));
	float rangeAccuDependence = 0;
	
	rangeAccuDependence = (float)_activeChar.calcStat(Stats.RANGE_ACCU_DIST_BOOST, 0, target, null); //given in PERCENT
	
	if (rangeAccuDependence > 0)
	{
		int distToTarg = (int)Util.calculateDistance(_activeChar, target, false);
		
		if (distToTarg > 40 && distToTarg < 2000)
		{
			distToTarg += 40; //try with "-" next time
			distToTarg = (int) Math.pow(distToTarg, 1.15);
			finalAccuracy *= distToTarg * rangeAccuDependence + 1;
		}
	}
...
public int getMaxHp()
{
	...
	if (_activeChar instanceof L2Summon)
	{
		...
		{
			if (_activeChar.getActingPlayer().isInOrcVillage())
				modifier = 0.7;
			else
				modifier = 1;
		}
		else if (_activeChar.getActingPlayer().isInOlympiadMode())
		{
