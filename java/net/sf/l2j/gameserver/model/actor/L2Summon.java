...
public boolean doDie(L2Character killer)
{
	...
	if (isPhoenixBlessed() && (getOwner() != null && !(CTF._started && getOwner()._inEventCTF)))
...
{
	final int basePAtk = super.getPAtk(target);
	final int overallPAtk = basePAtk + (_owner.getPAtk(target)/Config.SUMMON_PATK_MODIFIER);
	
	return (int) _owner.calcStat(Stats.SUMMON_PATK_BOOST, overallPAtk, this, null);
}

@Override
public int getMAtk(L2Character target, L2Skill skill)
{
	final int baseMAtk = super.getMAtk(target, skill);
	final int overallMAtk = baseMAtk + (_owner.getMAtk(target, skill)/Config.SUMMON_MATK_MODIFIER);
	
	return (int) _owner.calcStat(Stats.SUMMON_MATK_BOOST, overallMAtk, this, null);
}

@Override
public byte getAttackElement()
{
	final byte ownerElementType = _owner.getAttackElement();
	
	return ownerElementType;
}

@Override
public int getAttackElementValue(byte value)
{	
	final int ownerElementValue = _owner.getAttackElementValue(value);
	
	return ownerElementValue;
}
}
