			...
			if (target instanceof L2Attackable)
			{
				try
				{
					if (!_owner.isGM() && target.getAI().getAttackTarget() != null)
					{
						...
						{
							if ((_owner.getActingPlayer().getClanId() == 0 && mobAttackTarget.getActingPlayer().getClanId() == 0) ||
									_owner.getActingPlayer().getClanId() != mobAttackTarget.getActingPlayer().getClanId())
							{
								if (_owner.getParty() == null)
									return;
