...
{
private static final int[] TRANSFORMATION_ALLOWED_SKILLS = {3, 8, 9, 10, 18, 22, 28, 33, 65, 67, 78, 86, 98, 110, 144, 190, 196, 197, 223, 278, 279, 283, 912, 9009, 289, 293, 320, 361, 362, 400, 401, 402, 403, 404, 406, 407, 437, 440, 449, 494,
	539, 540, 559, 560, 561, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574, 575, 576, 577, 578, 579, 580, 581, 582, 583, 584, 585, 586, 587, 588, 589,
	619, 629, 630, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685, 686, 687, 688, 689, 690, 691, 692, 693, 694, 695, 696, 697, 698, 699,
	...
	738, 739, 740, 741, 745, 746, 747, 748, 749, 750, 751, 752, 753, 754, 795, 796, 797, 798,
	814, 815, 816, 817, 838, 839, 884, 885, 886, 887, 888, 891, 896,897,898,899,900,901,902,903,904,905,906,907,908,909,910,911,
	1015, 1016, 1018, 1027, 1028, 1034, 1042, 1043, 1201, 1206, 1217, 1264, 1266, 1360, 1361, 1400, 1409, 1411, 1417, 1418, 1430, 1303, 1059, 1471, 1472,
	1342, 1343,
	1508, 1514, 1515, 1523, 1524, 1525, 1528, 1532, 1533,
	3328, 3329, 3330, 3331, 3630, 3631,
	5437, 5491, 5655, 5656, 5657, 5658, 5659,
	8248, 9011, 9012, 16500, 16501, 16502, 16503,
	30040
};

public static final int[] HERO_SKILLS = {395, 396, 1374, 1375, 1376, 12504, 12503, 12502, 12505, 12501, 12506, 12511, 12509, 12508, 12510};
...
private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,fame=?,pvpkills=?,pkkills=?,rec_have=?,rec_left=?,clanid=?,race=?,classid=?,deletetime=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,punish_level=?,punish_timer=?,newbie=?,nobless=?,power_grade=?,subpledge=?,last_recom_date=?,lvl_joined_academy=?,apprentice=?,sponsor=?,varka_ketra_ally=?,clan_join_expiry_time=?,clan_create_expiry_time=?,death_penalty_level=?,bookmarkslot=?,streak=?,lastKill1=?,lastKill2=?,vitality_points=?,heroWpnDel=?,cancraft=? WHERE charId=?";
private static final String RESTORE_CHARACTER = "SELECT account_name, charId, char_name, level, maxHp, curHp, maxCp, curCp, maxMp, curMp, face, hairStyle, hairColor, sex, heading, x, y, z, exp, expBeforeDeath, sp, karma, fame, pvpkills, pkkills, clanid, race, classid, deletetime, cancraft, title, rec_have, rec_left, accesslevel, online, char_slot, lastAccess, clan_privs, wantspeace, base_class, onlinetime, isin7sdungeon, punish_level, punish_timer, newbie, nobless, power_grade, subpledge, last_recom_date, lvl_joined_academy, apprentice, sponsor, varka_ketra_ally,clan_join_expiry_time,clan_create_expiry_time,death_penalty_level,bookmarkslot,streak,lastKill1,lastKill2,vitality_points,heroWpnDel FROM characters WHERE charId=?";

//Character SQL Kill Count:
private static final String INSERT_KILLCOUNT = "INSERT INTO killcounter (charid,targetid,killcount) values (?,?,?)";
private static final String UPDATE_KILLCOUNT = "UPDATE killcounter SET killcount=? WHERE charid=? AND targetid=?";
private static final String RESTORE_KILLCOUNT = "SELECT charid,targetid,killcount FROM killcounter WHERE charId=?";
...
private boolean _inGMPvPEvent = false;

public boolean isInGMPvPEvent()
{
	return _inGMPvPEvent;
}
public void setInGMPvPEvent(boolean value)
{
	_inGMPvPEvent = value;
}

private int _GMPvPEventKillModifier = 10;
...
private final List<Integer> _killcounterChars = new FastList<Integer>();
...
public void startKickFromEventTask()
{
	...
	eventTicker = 0;
	_kickFromEventTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new kickFromEventTask(this), 2000, 80000);
}
...
public void warnAFK()
{
	sendMessage("You've been afk for 80 seconds! You will be kicked in another 80 seconds if you don't participate in the event!");
}
...
public void run()
{
	if (noob != null && noob._equipmentViewer == 0)
	{
		try
		{
...
public boolean isSpawnProtected() { return _protectEndTime > GameTimeController.getGameTicks() && getKarma() == 0; }
...
public int _countTvTkills, _countTvTdies, _originalKarmaTvT;
...
public int _originalKarmaCTF, _countCTFflags;
...
public int _countDMkills, _originalKarmaDM;
...
public int getRelation(L2PcInstance target)
{
	...
	final boolean fos = FOS._started && _inEventFOS && target._inEventFOS;
...
@Override
public final int getLevel() { return getLevel(false); }
...
public void increaseKillcount(L2PcInstance target)
{
	...
	try
	{
		...
		if (getKillCount() == 1)
		{
			statement = con.prepareStatement(INSERT_KILLCOUNT);
			statement.setInt(1, getObjectId());
			statement.setInt(2, target.getObjectId());
			statement.setInt(3, getKillCount());
			statement.execute();
			statement.close();
		}
		else if (getKillCount() > 1)
		{
			statement = con.prepareStatement(UPDATE_KILLCOUNT);
			statement.setInt(1, getKillCount());
			statement.execute();
			statement.close();
		}
		else
		{
			_log.warning("Could not store killcount. its NULL");
		}
	}		
	catch (Exception e)
	{
		_log.log(Level.SEVERE, "Failed inserting data to killcounter.", e);
	}
	finally
	{
		try { con.close(); } catch (Exception e) {}
	}			
	_killcounterChars.add(target.getObjectId());
}
...
public void giveMarriageSkills()
{
	if (isThisCharacterMarried())
	{
		...
		if (difference > 172800000) // 2 days
		{
				...
				if (difference > 691200000) // 8 days
				{
					...
					if (difference > 2764800000L) // 32 days
					{
						...
						if (difference > 5529600000L) //  64 days
						{
							...
							if (difference > 11059200000L) //  128 days
							{
...
final static public int[] PVP_SKILLS = {7045, 426, 427, 7046, 7041, 7042, 7049, 7054, 7050, 7056,
	7055, 7064, 15000, 15001, 15002, 15003, 15004, 15005, 15006, 15007, 15008, 15009, 15010};

final private void givePvpSkills()
{
...
final static public int[] PVP_TRANSFORM_SKILLS = {2428, 617, 3337, 674, 2631, 670, 2394, 618, 2670, 2671, 3336, 671, 3335, 673, 672, 5655, 546, 2511, 2632, 8246, 552, 555, 5261};
...
public void standUp()
{
	...
	else if (TvT._sitForced && _inEventTvT || FOS._sitForced && _inEventFOS || CTF._sitForced && _inEventCTF || DM._sitForced && _inEventDM || VIP._sitForced && _inEventVIP || _isSitForcedTvTi)
...
public void onAction(L2PcInstance player)
{
	...
	if (((TvT._started && !Config.TVT_ALLOW_INTERFERENCE) || (FOS._started && !Config.FortressSiege_ALLOW_INTERFERENCE) || (CTF._started && !Config.CTF_ALLOW_INTERFERENCE)
			|| ((player._inEventTvTi || _inEventTvTi) && !Config.TVTI_ALLOW_INTERFERENCE) || (DM._started && !Config.DM_ALLOW_INTERFERENCE) || (VIP._started && !Config.VIP_ALLOW_INTERFERENCE)) && !player.isGM())
	{
...
public void doAutoLoot(L2Attackable target, L2Attackable.RewardItem item)
{
	...
	{
		if (item.getItemId() == 4357) //silver
		{
			if (getInventory().getItemByItemId(4357) != null && getInventory().getItemByItemId(4357).getCount() >= 30000)
			{
				sendMessage("You reached the limit of Silver that you can farm. Return to town and deposit the Silver in your warehouse");
				return;
			}
		}
		else if (item.getItemId() == 4355) // blue eva
		{
			if (getInventory().getItemByItemId(4355) != null && getInventory().getItemByItemId(4355).getCount() >= 12000)
			{
				sendMessage("You reached the limit of Blue Eva that you can farm. Return to town and deposit the BE in your warehouse");
				return;
			}
		}
		else if (item.getItemId() == 961 || item.getItemId() == 962 || item.getItemId() == 959) // Crystal Scrolls, Normal Weapon Scrolls
		{
			if ((getInventory().getItemByItemId(961) != null && getInventory().getItemByItemId(961).getCount() >= 150) || (getInventory().getItemByItemId(962) != null && getInventory().getItemByItemId(962).getCount() >= 150) || (getInventory().getItemByItemId(959) != null && getInventory().getItemByItemId(959).getCount() >= 150))
			{
				sendMessage("You reached the limit of Enchant Scrolls that you can farm. Return to town and deposit them in your warehouse");
				return;
			}
		}
	}
	...
	try
	{
		...
		else if (item.getItemId() == 98022)
		{
			if (getClan() != null)
			{
				getClan().setReputationScore(getClan().getReputationScore()+item.getCount(), true);
				getClan().broadcastToOnlineMembers("Clan reputation increased by "+item.getCount()+" with the help of "+ getName()+"!");
			}
		}
		else addItem("Loot", item.getItemId(), item.getCount(), target, true, item.getEnchantLevel());
	}
	catch (Exception e)
	{
...
protected void doPickupItem(L2Object object)
{
	...
	else
	{
		...
		if (isInParty()) getParty().distributeItemPickUp(this, target);
		// Target is adena
		...
		else addItem("Pickup", target, this, true);
	}
}
...
public boolean doDie(L2Character killer)
{
	...
	if (killer != null)
	{
		...
		if (_streak < 60)
		{
			if (_tempHero) //killing streak hero
			{
				stopTempHeroTask();
...
private void onDieDropItem(L2Character killer)
{
	...
	if ((!isInsideZone(ZONE_PVP) || pk == null) && (!isGMReally() || Config.KARMA_DROP_GM))
	{
		...
		if (dropPercent > 0 && Rnd.get(100) < dropPercent)
		{
			...
			{
				if (itemDrop == null || itemDrop.getItemId() == 70100) continue;
...
final private void increasePvpKills(final L2PcInstance target, final boolean bonus, final boolean synergy, final String name, final int fameCarriedOver)
{
	...
	if (getAllyId() != 0 && getAllyId() == target.getAllyId() && !isInFunEvent())
		return;

	if (getClanId() != 0 && getClanId() == target.getClanId() && !isInFunEvent())
		return;
	...
	if (getIP() != null && target.getIP() != null && !getIP().equals(target.getIP()))
	{
		...
		try {
			if (synergy && gearScoreStatus < 3)
			{
				...
				case 7:
				case 9:
					sendMessage("Synergy kill from "+name+"!");
					break;
				case 1:
					sendMessage("You helped "+name+" kill "+target.getDisplayName()+", so the server pretends you got the kill too!");
					break;
				case 2:
					sendMessage("Only on our server does a server take care of support classes like this."); //LOL
					break;
				case 3:
					sendMessage("Healers, Tanks, Judicators, and Doomcryers are considered support classes (Untransformed).");
					break;
				case 4:
					sendMessage(name+" helped you help him!");
					break;
				case 8:
					sendMessage("You support "+name+", "+name+" supports you!");
					break;
				}
			...
			if ((!isInOrcVillage() && !target.isInOrcVillage()) || _famestreak < 10)
				_famestreak++;
			
			if (gearScoreStatus < 3)
			{
				_streak++;               
			    if (_streak == 25)
				{
			...
			if (!isInFunEvent())
			{
				...
				if (war || gearScoreStatus < 2)
				{
					...
					if (war || fameIncrease >= 2)
					{
						int decreasedFame = 0;

						if (fameIncrease >= 3)
							decreasedFame = Math.min(fameIncrease, 14)/2;

						if (target.getFame() >= 1700)
							decreasedFame += Math.pow(target.getFame(), 1/3);

						if (decreasedFame > fameIncrease+1)
							decreasedFame = fameIncrease+1;
						...
						if (!synergy && decreasedFame > 0)
							target.setFame(Math.max(0, target.getFame() - decreasedFame));
			...
			if (!synergy && !isSupportClass())  //start synergy kills
			{
				L2Party party = getParty();

				if (party != null)
				{
					for (L2PcInstance partymate : party.getPartyMembers())
					{
						if (partymate == null) continue;

						if (isInFunEvent() || partymate.isInFunEvent())
						{
							if (CanAttackDueToInEvent(partymate))
								continue;
						}

						if (partymate.isSupportClass() && getInSameClanAllyAs(partymate) > 0 && partymate.isInCombat())
						{
							if (partymate.isInsideRadius(this, 1100, false, false))
							{
								if (Rnd.nextInt(100) < partymate.getSupportIncreasePvPChance())
								{
									partymate.increasePvpKills(target, false, true, getName(), fameIncrease);
									break;
								}
							}
						}
			...
			if (getKarma() == 0 && gearScoreStatus < 2)
...
private static int[] ORDER = {1, 2, 4, 5, 6};

private void givePvpToken(L2PcInstance target)
{
	if (target == null)
		return;
	...
	int point = Rnd.get(100);

	if (point < 60) //60% is armor
	{
		if (target.getChestArmorInstance() != null)
			grade = target.getChestArmorInstance().getCrystalType();
	}
	else
	{
		if (target.getSecondaryWeaponInstance() != null) //using a shield
		{
			if (point < 90)
			{
				if (target.getActiveWeaponInstance() != null)
					grade = target.getActiveWeaponInstance().getCrystalType();
			}
			else
			{
				grade = target.getSecondaryWeaponInstance().getCrystalType();
			}
		}
		else //not using a shield
		{
			if (target.getActiveWeaponInstance() != null)
				grade = target.getActiveWeaponInstance().getCrystalType();
		}
	...
	boolean rare = !isInFunEvent() && !isInsideZone(L2Character.ZONE_TOWN) && Rnd.get(13000) < chance - 4;
...
{
	if (isInFunEvent() || isInsideZone(L2Character.ZONE_CHAOTIC))
		return;
	...
	else if ( newKarma > karmaLimit ) newKarma = karmaLimit;
	...
	if (!punish)
	{
		...
		{
			newKarma *= 2;
			//setPunishLevel(L2PcInstance.PunishLevel.JAIL, 60);
		}
	}
	else
	{
...
public void updatePvPStatus(L2Character target)
{
	...
	if (target instanceof L2Npc)
	{
		...
		{
			L2NpcTemplate boss = NpcTable.getInstance().getTemplate(((L2Npc)target).getNpcId());
			FakePc fpc = FakePcsTable.getInstance().getFakePc(((L2Npc)target).getNpcId());

			if ((fpc != null && fpc.pvpFlag > 0) || (boss != null && boss.isLevelFourRB()))
			{
				if (isInDuel())
					return;
...
public boolean mountPlayer(L2Summon pet)
{
	if (pet != null && pet.isMountable() && !isMounted() && !isBetrayed())
	{
		...
		else if (isTransformed() || isCursedWeaponEquipped() || isInOrcVillage() || isInHellbound())
		{
...
private static L2PcInstance restore(int objectId)
{
	...
	try
	{
		...
		statement = con.prepareStatement("SELECT lockdowntime, secret FROM accounts WHERE login=?");
...
private void restoreCharData()
{
	...
	// Retrieve from the database all macros of this L2PcInstance and add them to _macroses.
	_macroses.restore();
	...
	// Retrieve from the database all teleport book mark of this L2PcInstance and add them to _tpbookmark.
	restoreTeleportBookmark();
    
	//Restores Killcount from the database
	restoreKillcount();
...
private void restoreKillcount()
{
	...
	{
		con = L2DatabaseFactory.getInstance().getConnection();
		PreparedStatement statement = con.prepareStatement(RESTORE_KILLCOUNT);
		statement.setInt(1, getObjectId());
		ResultSet rset = statement.executeQuery();
		while (rset.next())
		{
			_killcounterChars.add(rset.getInt("targetid"));
		}
		rset.close();
		statement.close();
	}
	...
	{
		_log.log(Level.SEVERE, "Failed restoring character killcount.", e);
	}
	finally
	{
		try { con.close(); } catch (Exception e) {}
...
public boolean isAutoAttackable(L2Character attacker)
{
	...
	if (attacker instanceof L2Playable)
	{
		...
		if (PCattacker == null) return false;

		if (!attacker.isGM() && (isInvisible() && isGM())) return false;
...
public boolean isAutoAttackableSwitch(L2PcInstance attacker)
{
	...
	if (TvT._started || FOS._started || CTF._started || VIP._started) // when events started it's slightly different
	{
		...
		else if (attacker._inEventFOS && _inEventFOS)
		{
			...
			if (attacker._inEventCTF && _inEventCTF)
			{
				...
				if (attacker._inEventVIP && _inEventVIP)
				{
...
public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
{
	...
	if (!isGM())
	{
		if (skill.getTransformId() > 0)
		{
			...
			default:
				...
				{
					sendPacket(ActionFailed.STATIC_PACKET);
					sendMessage("Cannot use transformation in Orc village");
					return;
				}
	...
	else if (skill.getSkillType() == L2SkillType.RESURRECT || skill.getId() == 438 || skill.getId() == 1410)
	{
		...
		{
			sendMessage("Your group has already used up all "+getCurrentInstance().getResLimit()+
...
private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
{
	...
	if ((sklTargetType == SkillTargetType.TARGET_HOLY && !FOS.checkIfOkToCastSealOfRule(this) && !checkIfOkToCastSealOfRule(CastleManager.getInstance().getCastle(this), false))
...
{
	return FOS.checkIfOkToCastSealOfRule(this);
}
...
public final boolean checkAOEPvPSkill(L2PcInstance target, L2Skill skill)
{
	if (target != null)
	{
		if (!isGM())
		{
			...
			if (target.getDuelState() == Duel.DUELSTATE_DUELLING
			...
			if (getParty() != null && target.getParty() != null
...
/**
 * Return the modifier corresponding to the Enchant Effect of the Active Weapon (Max : 127).<BR><BR>
 */
...
public boolean addSubClass(int classId, int classIndex)
{
	...
	try
	{
		...
		{
			_log.warning("WARNING: Could not add character sub class for " + getName() + ": "
					+ e);
			return false;
		}
		finally
...
public boolean setActiveClass(int classIndex)
{
	...
	try
	{
		...
		for (L2Character character : getKnownList().getKnownCharacters())
		{
			if (character.getFusionSkill() != null
					&& character.getFusionSkill().getTarget() == this)
				character.abortCast();
		}
		...
		else
		{
			...
			{
				_log.info("Could not switch " + getName() + "'s sub class to class index "
						+ classIndex + ": " + e);
				return false;
			}
		}
...
/*private boolean _allowSub = true;

final public boolean allowSubSinceInstIsOver()
{
	return _allowSub;
}
final public void setAllowSubSinceInstIsOver(final boolean val)
{
	_allowSub = val;
}
...
private boolean cannotChangeSubsDueToInstance()
{
	...
	if (world != null)
	{
		if ((world.templateId == 2000 || world.templateId == 2001) && !allowSubSinceInstIsOver())
			return true;
	}
...
public void onPlayerEnter()
{
	...
	if (_isInvul)
		sendMessage("You are invulnerable");
	if (isInvisible())
		sendMessage("You are invisible");
	if (getMessageRefusal())
		sendMessage("Message Refusal mode");
...
public void addExpAndSp(long addToExp, int addToSp, boolean useVitality) { getStat().addExpAndSp(addToExp, addToSp, useVitality); }
...
private void givePvpExp(L2PcInstance target, int streakill) //custom edit
{
	...
	// Set the new Experience value of the L2PcInstance
	getStat().addExpAndSp(gainedExp, 40000);
...
private final boolean isSkillDisabledDueToTransformation(int skillId)
{
	...
	if (isTransformed() || isInStance())
	{
		...
		if (getTransformationId() == 106 || getTransformationId() == 109 || getTransformationId() == 110) //horse, beatle, lion
		{
...
{
	if (!_inEventVIP || !player._inEventVIP) return false;
	if (_isVIP && player._isVIP) return true;
	if (_isNotVIP && player._isNotVIP) return true;
...
private boolean _isInGiran = false;

public void setIsInGiran(final boolean val)
{
	_isInGiran = val;
}
public boolean isInGiran()
{
	return _isInGiran;
}
...
public void setNameColorsDueToPVP()
{
	...
	if (getPvpKills() >= 500 && getPvpKills() <= 999)
	{
		getAppearance().setNameColor(Integer.decode("0xFFC98C"));
	}
	else if (getPvpKills() >= 1000 && getPvpKills() <= 1999)
	{
...
public void useItem(final int itemObjectId, final boolean force)
{
	...
	if (item.isEquipable())
	{
		...
		if(NexusEvents.isInEvent(this))
		{
			NexusEvents.onUseItem(this, item);
		}
	}
	else
	{
...
public void storeName()
{
	...
	catch (Exception e) { _log.warning("Could not store char name: "+ e); }
	finally { try { con.close(); } catch (Exception e) {} }
}
...
public void wipeHeroOlyStatsDatabase()
{
	...
	catch (Exception e) { _log.warning("Could not wipe char herooly data "+ e +getName()); }
	finally { try { con.close(); } catch (Exception e) {} }
}
...
public static void wipeHeroOlyStatsDatabase(int objId)
{
	...
	catch (Exception e) { _log.warning("Could not wipe char herooly2 data "+ e +objId); }
	finally { try { con.close(); } catch (Exception e) {} }
}
...
// Formula for Gear Level Calculation by Meydex
public float getGearLevel()
{
	float finalGearLevel = 0;
	
	for (L2ItemInstance item : getInventory().getItems())
	{	
		float combinedGearLevel = (item.isAugmented() ? 2 : 0) + (item.getUniqueness() * (item.isHairAccessory() ? 6 : item.isDread() ? (float)item.getEnchantLevel() + 3 : (float)item.getEnchantLevel()));
		if (item != null && item.isEquipped())
		{		   
			finalGearLevel = combinedGearLevel + finalGearLevel;
		}
	}
    return finalGearLevel;
}

public String getGearLevelRarityColor()
{
	if(getGearLevel() >= 150 && getGearLevel() <= 199) //Light Blue
	   return "00FFFF";
	else if(getGearLevel() >= 200 && getGearLevel() <= 399) //Green
	   return "00FF00";
	else if(getGearLevel() >= 400 && getGearLevel() <= 599) //Yellow
		   return "FFFF00";
	else if(getGearLevel() >= 600 && getGearLevel() <= 699) //Orange
		   return "FF9933";
	else if(getGearLevel() >= 700) //Red
		   return "FF0000";
