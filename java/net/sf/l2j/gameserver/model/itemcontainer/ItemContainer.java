...
public L2ItemInstance transferItem(String process, int objectId, long count, ItemContainer target, L2PcInstance actor, L2Object reference)
{
	...
	/*else if (actor.getAccountName() == "angeldragon" || actor.getAccountName() == "thedarga1")
	{
		try
		{	String command = target.getClass().getSimpleName();
		String targetName = "";
	...
	else if (reference != null && reference instanceof L2PcInstance && (reference.getActingPlayer().getAccountName() == "angeldragon" || reference.getActingPlayer().getAccountName() == "thedarga1"))
	{
		try
		{	String command = target.getClass().getSimpleName();
		String targetName = "";
...
public void restore()
{
	...
	try
	{
		...
		PreparedStatement statement = con.prepareStatement("SELECT object_id, item_id, count, enchant_level, loc, loc_data, custom_type1, custom_type2, mana_left, time, trade_time, instance FROM items WHERE owner_id=? AND (loc=?)");
