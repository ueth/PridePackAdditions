...
{
	return IconTable.getInstance().getIcon(itemId);
}

public TextBuilder equipInfo()
{
	TextBuilder html1 = new TextBuilder();
	html1.append("<center><table border=0 width=\"26%\"><tr><td><font color=AFACEC>[Player Equipment]</font></td></tr></table>");
	html1.append("<table cellspacing=7 cellpadding=0 border=0 width=\"85%\">"); //default 76%
	...
	else
	{
		...
		for (Integer slot : ORDER)
		{
			...
			if (item != null)
			{
				...
				itemIcon = getItemIcon(item.getItemId());
			...
			if (odd)
			{
				html1.append("<center><tr valign=\"middle\"><td align=\"right\"><font color=\"LEVEL\" align=\"right\">"+slotName+":</font></td><td><img src=\""+ itemIcon +"\" height=32 width=32></td><td><font color=\""+color+"\">"+ itemName + "</font></td>");
				odd = false;
			}
			else
			{
				html1.append("<td align=\"right\"><font color=\"LEVEL\" align=\"right\">"+slotName+":</font></td><td><img src=\""+ itemIcon + "\" height=32 width=32></td><td><font color=\""+color+"\">"+ itemName + "</font></td></tr></center>");
				odd = true;
			}
		}
	}
...
public void restore()
{
	...
	try
	{
		...
		PreparedStatement statement = con.prepareStatement("SELECT object_id, item_id, count, enchant_level, loc, loc_data, custom_type1, custom_type2, mana_left, time, trade_time, instance FROM items WHERE owner_id=? AND (loc=? OR loc=?) ORDER BY loc_data");
