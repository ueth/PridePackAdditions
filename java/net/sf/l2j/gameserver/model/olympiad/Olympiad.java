...
private static final String OLYMPIAD_GET_HEROS = "SELECT olympiad_nobles.charId, characters.char_name "
		+ "FROM olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId "
		+ "AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= 20 "
...
private static final String GET_ALL_CLASSIFIED_NOBLESS = "SELECT charId from olympiad_nobles_eom "
		+ "WHERE competitions_done >= 20 ORDER BY olympiad_points DESC, competitions_done DESC";
private static final String GET_EACH_CLASS_LEADER = "SELECT characters.char_name from olympiad_nobles_eom, characters "
		...
		+ "AND olympiad_nobles_eom.competitions_done >= 20 "
...
private static final String GET_EACH_CLASS_LEADER_CURRENT = "SELECT characters.char_name from olympiad_nobles, characters "
		...
		+ "AND olympiad_nobles.competitions_done >= 20 "
...
public boolean registerNoble(L2PcInstance noble, boolean classBased)
{
	...
	if (noble._inEventTvT || noble._inEventFOS || noble._inEventCTF || noble._inEventDM || noble._inEventVIP || noble.getInstanceId() != 0)
	{
	...
	if (noble.getLevel() < 90)
	{
		noble.sendMessage("You must be level 90 or higher to participate in the Olympiad.");
		return false;
	}
	if (noble.getPvpKills() < 500)
	{
		noble.sendMessage("You must have at least 500 PvPs to participate in the Olympiad.");
		return false;
	}
...
public static void addSpectator(int id, L2PcInstance spectator, boolean storeCoords)
{
	...
	if (spectator._inEventCTF || spectator._inEventFOS || spectator._inEventTvT || spectator._inEventVIP || spectator._inEventDM)
	{
