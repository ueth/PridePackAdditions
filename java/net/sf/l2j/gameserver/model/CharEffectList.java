...
final private void addEffectFromQueue(final L2Effect newEffect)
{
	...
	if (newSkill.isDebuff())
	{
		...
		if (newEffect.getEffectType() == L2EffectType.DEBUFF || newEffect.getEffectType() == L2EffectType.DMG_OVER_TIME || newEffect.getEffectType() == L2EffectType.STUN || newEffect.getEffectType() == L2EffectType.PARALYZE ||
				newEffect.getEffectType() == L2EffectType.MANA_DMG_OVER_TIME || newEffect.getEffectType() == L2EffectType.CHANCE_SKILL_TRIGGER
				|| newEffect.getEffectType() == L2EffectType.SILENCE_MAGIC_PHYSICAL || newEffect.getEffectType() == L2EffectType.MUTE)
		...
		for (L2Effect e : _debuffs)
		{
			...
			if (count >= 0)
			{
				if (e.getEffectType() == L2EffectType.DEBUFF || e.getEffectType() == L2EffectType.DMG_OVER_TIME || newEffect.getEffectType() == L2EffectType.STUN || newEffect.getEffectType() == L2EffectType.PARALYZE ||
						e.getEffectType() == L2EffectType.MANA_DMG_OVER_TIME || e.getEffectType() == L2EffectType.CHANCE_SKILL_TRIGGER
						|| newEffect.getEffectType() == L2EffectType.SILENCE_MAGIC_PHYSICAL || newEffect.getEffectType() == L2EffectType.MUTE)
				{
					count++;
	...
	else
	{
		...
		for (L2Effect e : _buffs)
		{
			if (e != null
					...
					&& e.getEffectType() == newEffect.getEffectType()
					&& e.getStackOrder() == newEffect.getStackOrder()
					&& e.getStackType().equalsIgnoreCase(newEffect.getStackType()))
			{
				e.exit(); // exit this
			}
		}
