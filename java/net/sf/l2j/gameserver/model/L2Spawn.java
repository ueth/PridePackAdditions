...
/**
 * Return the delay between a L2NpcInstance remove and its re-spawn in miliseconds.<BR><BR>
 */
...
public void decreaseCount(/*int npcId*/L2Npc oldNpc)
{
	...
	if (_doRespawn && _scheduledCount + _currentCount < _maximumCount )
	{
		...
		if (doit)
		{
			...
			if (respawnDelayMulti > 1.2)
				respawnDelayMulti = 1.2;
			else if (respawnDelayMulti < 0.25)
				respawnDelayMulti = 0.25;
		...
		_respawnTask =*/ ThreadPoolManager.getInstance().scheduleGeneral(new SpawnTask(oldNpc), delay);
	}
}
...
public L2Npc doSpawn(boolean isSummonSpawn)
{
	...
	try
	{
		...
		if (_template.type.equalsIgnoreCase("L2Pet") || _template.type.equalsIgnoreCase("L2Minion") || _template.type.equalsIgnoreCase("L2FlyMinion")
...
private L2Npc intializeNpcInstance(L2Npc mob)
{
	...
	if (getSpawnType() == 1)
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
		{
			...
			{
				if (player.isInsideZone(L2Character.ZONE_PEACE) || (player._inEventDM && DM._started))
					continue;
