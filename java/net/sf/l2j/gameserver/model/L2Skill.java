...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	{
		TARGET_NONE, TARGET_SELF, TARGET_ONE, TARGET_PARTY, TARGET_ALLY, TARGET_CLAN, TARGET_PET, TARGET_SUMMON, TARGET_AREA, TARGET_FRONT_AREA, TARGET_BEHIND_AREA, TARGET_AURA, TARGET_ALL, TARGET_FRONT_AURA, TARGET_BEHIND_AURA, TARGET_CORPSE, TARGET_UNDEAD, TARGET_AREA_UNDEAD, TARGET_MULTIFACE, TARGET_CORPSE_ALLY, TARGET_CORPSE_CLAN, TARGET_CORPSE_PLAYER, TARGET_CORPSE_PET, TARGET_AREA_CORPSE_MOB, TARGET_CORPSE_MOB, TARGET_UNLOCKABLE, TARGET_HOLY, TARGET_FLAGPOLE, TARGET_SELF_AND_PET, TARGET_ONE_AND_PET, TARGET_PARTY_MEMBER, TARGET_PARTY_OTHER, TARGET_ENEMY_SUMMON, TARGET_OWNER_PET, TARGET_GROUND, TARGET_COUPLE, TARGET_SELF_AND_COUPLE, TARGET_CORPSE_DRAIN, TARGET_CORPSE_DRAIN_AOE, TARGET_TRAP
	}
		...
		// The skill can only be used on the L2Character targeted, or on the caster itself
			...
			case TARGET_PARTY:
			{
				...
				else
				{
					...
					if (player != null && (player.isInDuel() || player.isInOrcVillage()))
						return targetList.toArray(new L2Character[targetList.size()]);
	...
	public final L2Effect[] getEffects(L2Character effector, L2Character effected, Env env, boolean ignoreInvul)
	{
		...
		for (EffectTemplate et : _effectTemplates)
		{
			...
			if (isOffensive())
			{
				...
				{
					if (et.effectPower > -1)
						success = Formulas.calcEffectSuccess(effector, effected, et, this, env.shld);
				}
			}
