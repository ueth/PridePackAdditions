...
{
	if (_started || _teleport || _joining || _teams.size() > 2 || _teams.size() < 1 || _eventName.equalsIgnoreCase("")
			|| _joiningLocationName.equals("") || _eventDesc.equals("") || _npcId == 0 || _flagX == 0 || _flagY == 0
			|| _flagZ == 0 || _npcX == 0 || _npcY == 0 || _npcZ == 0 || _rewardId == 0 || _rewardAmount == 0
...
public static void showArtifactHtml(L2PcInstance eventPlayer, String objectId)
{
	...
	{
		System.out.println("FOS Engine[showArtifactHtml(" + eventPlayer.getName() + ", " + objectId + ")]: exception: "
				+ e.getStackTrace());
	}
}
...
public static void showEventHtml(L2PcInstance eventPlayer, String objectId)
{
		...
		else if (!_started && _joining && eventPlayer.getLevel() >= _minlvl && eventPlayer.getLevel() < _maxlvl)
		{
			if (_players.contains(eventPlayer) || _playersShuffle.contains(eventPlayer) || checkShufflePlayers(eventPlayer))
			{
				...
				replyMSG.append("<td width=\"60\"><center><button value=\"Remove\" action=\"bypass -h npc_" + objectId
						+ "_fos_player_leave\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center></td>");
			...
			else
			{
				...
				replyMSG.append("<td width=\"200\">Minimum level : <font color=\"00FF00\">" + _minlvl
						+ "</font></td><br>");
				replyMSG.append("<td width=\"200\">Maximum level : <font color=\"00FF00\">" + _maxlvl
						+ "</font></td><br><br>");
				...
				if (Config.FortressSiege_EVEN_TEAMS.equals("NO") || Config.FortressSiege_EVEN_TEAMS.equals("BALANCE"))
				{
					...
					for (String team : _teams)
					{
						...
						replyMSG.append("<td width=\"60\"><button value=\"Join\" action=\"bypass -h npc_" + objectId
								+ "_fos_player_join " + team
				...
				else if (Config.FortressSiege_EVEN_TEAMS.equals("SHUFFLE"))
				{
					...
					replyMSG.append("<button value=\"Join\" action=\"bypass -h npc_" + objectId
							+ "_fos_player_join eventShuffle\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
				...
				else if (Config.FortressSiege_EVEN_TEAMS.equals("RACIAL"))
				{
					...
					replyMSG.append("</table></center><br>");
					replyMSG.append("<button value=\"Join\" action=\"bypass -h npc_" + objectId
							+ "_fos_player_join eventRacial\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	...
	{
		_log.severe("FOS Engine[showEventHtml(" + eventPlayer.getName() + ", " + objectId + ")]: exception"
				+ e.getMessage());
	}
}
...
public static void finishEvent()
{
	try
	{
		...
		synchronized (topkiller)
		{
			for (L2PcInstance player : _players)
			{
				if (player != null && player._countFOSKills > 0 && (player._countFOSKills > _top1Kills || (player._countFOSKills >= _top1Kills && player._countFOSdies < topkiller[0]._countFOSdies)))
				{
					topkiller[0] = player;
			...
			for (L2PcInstance player : _players)
			{
				if (player != null && player._countFOSKills > 0 && player != topkiller[0] && (player._countFOSKills > _top2Kills || (player._countFOSKills >= _top2Kills && player._countFOSdies < topkiller[1]._countFOSdies)))
				{
					topkiller[1] = player;
			...
			for (L2PcInstance player : _players)
			{
				if (player != null && player._countFOSKills > 0 && player != topkiller[0] && player != topkiller[1] && (player._countFOSKills > _top3Kills || (player._countFOSKills >= _top3Kills && player._countFOSdies < topkiller[2]._countFOSdies)))
				{
					topkiller[2] = player;
...
public static void rewardTeam(String teamName)
{
	...
	for (L2PcInstance player : _players)
	{
		...
		{
			if (Config.FortressSiege_PRICE_NO_KILLS || player._countFOSKills > 0 || player._countFOSCaps > 0 || player._countFOSdies > 2)
			{
				if (teamName != null)
				{
					...
					if (player._teamNameFOS.equals(teamName))
					{
						...
						player.addItem("FOS Event: " + _eventName, TvT.MEDAL_OF_VICTORY, 1, player, true);
					}
					else //loser team
					{
						...
						NpcHtmlMessage nhm = new NpcHtmlMessage(5);
						TextBuilder replyMSG = new TextBuilder("");
						replyMSG.append("<html><body>Your team did not manage to capture any altars during the event. You're rewarded 1/4 the prize for trying.</body></html>");
						nhm.setHtml(replyMSG.toString());
						player.sendPacket(nhm);
					}
				}
...
public static void doSwap()
{
	...
	ThreadPoolManager.getInstance().scheduleGeneral(new Runnable() {// teleport players back to
		// reverse positions
		public void run()
		{
			sit();
...
{
	if ((Config.FortressSiege_EVEN_TEAMS.equals("SHUFFLE") && (_teleport || _started))
			|| (Config.FortressSiege_EVEN_TEAMS.equals("NO") || Config.FortressSiege_EVEN_TEAMS.equals("BALANCE")
					&& (_teleport || _started)))
	{
...
public static boolean teleportStart()
{
	...
	Announcements(_eventName + ": Teleport to team spot in 10 seconds!");
	setUserData();
	spawnDoors();
	spawnFlag();
	spawnBuffers();
			...
			if (player._inEventFOS && _started)
			{
				if (player.isDead())
				{
					if (teleToCentral && eventCenterX != 0)
						...
						player.teleToLocation(_teamsX.get(_teams.indexOf(player._teamNameFOS)), _teamsY.get(_teams.indexOf(player._teamNameFOS)), _teamsZ
								.get(_teams.indexOf(player._teamNameFOS)), false);
