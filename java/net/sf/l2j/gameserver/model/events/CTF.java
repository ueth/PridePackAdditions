...
public synchronized static void processInFlagRange(L2PcInstance _player)
{
	try
	{
		CheckRestoreFlags();
		...
		for (String team : _teams)
		{
			if (team.equals(_player._teamNameCTF))
			{
				...
				if (InRangeOfFlag(_player, indexOwn, 100) && !_flagsTaken.get(indexOwn) && _player._haveFlagCTF)
				{
					...
					_player.addItem("CTF: " + CTF._eventName, 4357, 40, _player, true);
			...
			else
			{
				...
				if (InRangeOfFlag(_player, indexEnemy, 100) && !_flagsTaken.get(indexEnemy) && !_player._haveFlagCTF && !_player.isDead())
				{
					...
					{
						_player.sendMessage("Dismount first");
						break;
					}
	...
	for (L2PcInstance player : _players)
	{
		if (player != null && player.isOnline() == 1 && player._inEventCTF)
		{
			if (player._teamNameCTF.equals(teamName))
			{
				...
				if (!tied)
				{
					player.addItem("CTF Event: " + _eventName, TvT.MEDAL_OF_VICTORY, 1, player, true);
				}
			}
			else
			{
		...
		public void run()
		{
			if (player._inEventCTF && _started)
			{
				if (player.isDead())
				{
					player.setInstanceId(FOS.SIEGE_EVENT_INSTANCE_ID);
					player.teleToLocation(_teamsX.get(_teams.indexOf(player._teamNameCTF)), _teamsY.get(_teams.indexOf(player._teamNameCTF)), _teamsZ
							.get(_teams.indexOf(player._teamNameCTF)), false);
					player.doRevive();
