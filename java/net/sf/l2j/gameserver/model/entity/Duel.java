...
public void teleportBack()
{
	if (_paDuel)
	{
		...
		_player.stopAbnormalEffect(AbnormalEffect.HOLD_1);
...
public void run()
{
	try
	{
		...
		if (count == 4)
		{
			...
			_duel.teleportPlayers(21319, 252948, -2022, 17634, 253958, -2022);
...
/**
 * Stops all players and summons from attacking.
...
private void stopFighting()
{
	...
	L2Summon summonA = null;
	L2Summon summonB = null;
	
	if (_playerA.getPet() != null)
	{
	    summonA = _playerA.getPet();
	}
	
	if (_playerB.getPet() != null)
	{
	    summonB = _playerB.getPet();
	}
	...
	if (_partyDuel)
	{
		for (L2PcInstance temp : _playerA.getParty().getPartyMembers())
		{
			...
			//Flag after duel fix on summons
			if(temp.getPet() != null)
			{
				temp.getPet().abortAttack();
	...
	else
	{
		...
		if(_playerA.getPet() != null)
		{
			summonA.abortAttack();
			summonA.abortCast();
			summonA.getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
		}
		...
		if(_playerB.getPet() != null)
		{
			summonB.abortAttack();
			summonB.abortCast();
			summonB.getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
		}
	}
...
public void teleportPlayers(int x, int y, int z, int x2, int y2, int z2)
{
	if (!_partyDuel)
		return;
	...
	for (L2PcInstance temp : _playerA.getParty().getPartyMembers())
	{
		if (temp == null)
			continue;
		temp.teleToLocation(x , y , z, true);
	}
	...
	for (L2PcInstance temp : _playerB.getParty().getPartyMembers())
	{
		if (temp == null)
			continue;
		temp.teleToLocation(x2, y2, z2, true);
	}
}
...
public void endDuel(DuelResultEnum result)
{
	...
	case Team1Win:
		...
		sm.addString(_playerA.getDisplayName());
	...
	case Team2Win:
		...
		if (_partyDuel)
			...
			sm = new SystemMessage(SystemMessageId.C1_HAS_WON_THE_DUEL);
		sm.addString(_playerB.getDisplayName());
