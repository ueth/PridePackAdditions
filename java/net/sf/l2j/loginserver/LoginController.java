...
/**
 * Adds the address to the hwid ban list of the login server, with the given duration.
 *
 * @param address The Address to be banned.
 * @param expiration Timestamp in miliseconds when this ban expires
 * @throws SocketException if the address is invalid.
 */
public void addHWIDBanForAddress(String address, long expiration) throws SocketException
{
	NetworkInterface hwid = NetworkInterface.getByName(address);
	if (!_bannedHwid.containsKey(hwid.getHardwareAddress()))
		_bannedHwid.put(hwid.getHardwareAddress(), new BanInfo(hwid, expiration));
}
 ...
 */
public void addHWIDBanForAddress(NetworkInterface address, long duration)
{
	try {
		if (!_bannedHwid.containsKey(address.getHardwareAddress()))
		_bannedHwid.put(address.getHardwareAddress(), new BanInfo(address, System.currentTimeMillis() + duration));
...
public boolean loginValid(String user, String password, L2LoginClient client)// throws HackingException
{
	...
	try
	{
		...
		if (expected == null)
		{
			...
			{
				if ((user.length() >= 2) && (user.length() <= 14) && !Pattern.matches(".*[�������������].*", user))
				{
				...
				_log.warning("Invalid username creation attempt: " + user);
				return false;
			}
...
public BanInfo(NetworkInterface hwid, long expiration) {
